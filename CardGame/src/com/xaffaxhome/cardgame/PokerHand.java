...
public class PokerHand extends CardHand
{
	...
	private static class PokerHandValidator
	{
		...
		{
			List<FrenchCard> tempHand = toValidate == null
					? this.pokerHandValidate
					: toValidate;
			...
			List<FrenchCard> uniqueCardRanks = tempHand.stream()
					.collect(Collectors.collectingAndThen(
							Collectors.toCollection(() -> new TreeSet<>(
									Comparator.comparing(FrenchCard::getRank))),
							ArrayList::new));
			...
			for (FrenchCard card : uniqueCardRanks)
			{
				...
				{
					if (card.getRank() == Rank.ACE && howManyinSequence == 4
							&& iterator == 5)
					{
						is_5highStrait = true;
		...
		private boolean checkFlush()
		{
			...
			for (Suit suit : Suit.values())
				suitCardMap.put(suit,
						this.pokerHandValidate.stream()
			...
			for (Map.Entry<Card.Suit, List<FrenchCard>> suitCards : suitCardMap
					.entrySet())
			{
				if (suitCards.getValue().size() < 5)
					continue;
				is_flush = true;
				currValue = suitCards.getValue().stream().limit(5)
						.mapToInt(r -> r.getRank().rank()).sum();
				...
				if (currValue > prevValue)
				{
					bestValue = temp_is_straight
							? currValue
									+ PokerHandValues.STRAIGHT_FLUSH.handValue()
											* MAIN_HAND_VALUE_COEFFICIENT
			...
			String handText = is_straight
					? "You have a straight flush with " + highCard.toString()
							+ " kicker."
					: "You have a flush with " + highCard.toString()
							+ " kicker.";
		...
		private boolean checkDuplicates()
		{
			...
			// ideally you would only iterate over pokerHandValidate.
			// not sure how to do this with a stream though :(.
			for (Card.Rank rank : Card.Rank.values())
			{
				long tempMultiples = this.pokerHandValidate.stream()
						.filter(r -> r.getRank().equals(rank)).count();
			...
			Card.Rank highest = Collections.max(
					rankFreqMap.get(Collections.max(rankFreqMap.keySet())));

			// kicker can be from the "1-of" set.
		...
		public Integer validateHand()
		{
			...
			if (is_straight && is_flush)
			{
				...
				this.pokerHandValue = tempValue + MAIN_HAND_VALUE_COEFFICIENT
						* PokerHandValues.STRAIGHT_FLUSH.handValue();
			}
